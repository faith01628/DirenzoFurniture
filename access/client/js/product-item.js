...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  // Extract the 'desc' query parameter from the URL
  const urlParams = new URLSearchParams(window.location.search);
  const descParam = urlParams.get('product'); // Lấy giá trị product từ URL

  if (!descParam) {
    console.warn('No product description provided in the URL.');
    return;
  }

  // Tạo body để truyền dữ liệu desc vào API
  const requestBody = {
    desc: descParam
  };
  ...
  fetch('https://furni1.transtechvietnam.com/search', {
    ...
    .then(data => {
      if (data.length > 0) {
        ...
        totalPages = Math.ceil(products.length / productsPerPage); // Set total pages
        displayPage(currentPage); // Display the first page
        updatePaginationControls(); // Update pagination controls
      } else {
        console.warn(`No products found with desc: ${descParam}`);
      }
    })
    .catch(error => {
...
function displayPage(page) {
  ...
  const productsToShow = products.slice(start, end); // Lấy sản phẩm cho trang hiện tại
...
function addProductToList(products) {
  ...
  products.forEach(product => {
    ...
    imageLink.href = `./view-product?id=${product.id}`;
